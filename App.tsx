...
function App() {
  ...
  // Verificar si la API_KEY está configurada (UX Feedback)
  ...
  const handleGenerate = async (prefs: UserPreferences) => {
    if (isApiKeyMissing) {
      setError("Falta la API_KEY de Google Gemini. Por favor, configúrala en las variables de entorno de Vercel para poder generar contenido.");
      return;
    }
    ...
    try {
      ...
      setError("Error de IA: " + (err.message || "Fallo al diseñar el programa. Intenta con un título más descriptivo."));
    } finally { 
  ...
  const handleImportJson = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        alert("Error: El archivo JSON no es un respaldo válido de ProfesorIA.");
        e.target.value = "";
      }
    };
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 flex flex-col font-['Inter']">
      ...
      {error && (
        <div className="fixed inset-0 z-[30000] bg-slate-950/90 flex items-center justify-center p-6 backdrop-blur-md">
          <div className="max-w-md w-full bg-slate-900 border border-red-500/30 p-10 rounded-[40px] text-center shadow-2xl">
            ...
            <h2 className="text-xl font-black text-white mb-4 uppercase">Aviso del Sistema</h2>
      ...
      {isGenerating && (
        <div className="fixed inset-0 z-[20000] flex flex-col items-center justify-center bg-slate-950/95 backdrop-blur-2xl p-10 text-center">
          ...
          <h2 className="text-2xl font-black text-white uppercase tracking-tighter mb-2">Diseñando Temario con IA...</h2>
          <p className="text-cyan-500 text-[10px] font-black uppercase tracking-[0.3em] animate-pulse">Estructurando Unidades y Competencias</p>
        </div>
      )}
      ...
      {!teacher ? (
        <div className="min-h-screen flex items-center justify-center p-6">
           <div className="glass-card p-12 rounded-[50px] max-w-sm w-full text-center">
              ...
              <input id="login-id" className="w-full p-5 rounded-2xl bg-slate-950 border border-white/5 mb-4 text-center text-white outline-none focus:border-cyan-500 transition-all" placeholder="ID Mindbox / Docente" />
              <button onClick={() => handleLogin((document.getElementById('login-id') as HTMLInputElement).value)} className="w-full py-5 bg-cyan-500 text-slate-950 rounded-2xl font-black uppercase text-xs shadow-lg hover:scale-105 transition-all">Iniciar Sesión</button>
           </div>
        </div>
      ) : currentCourse ? (
        ...
        <div className="max-w-7xl mx-auto w-full px-8 py-20">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-16 border-b border-white/5 pb-10 gap-6">
            <div>
              <p className="text-cyan-500 text-[10px] font-black uppercase tracking-widest mb-1">Bienvenido, {teacher.name}</p>
            ...
            <div className="flex flex-wrap gap-4">
              {isApiKeyMissing && (
                <div className="bg-amber-500/10 border border-amber-500/30 px-6 py-4 rounded-2xl flex items-center gap-3">
                  ...
                  <p className="text-[9px] font-black text-amber-500 uppercase">Falta API Key en Vercel</p>
                </div>
              )}
              ...
              <button onClick={() => setShowForm(true)} className="px-8 py-5 bg-white text-slate-950 rounded-[24px] font-black text-[10px] uppercase tracking-widest hover:bg-cyan-500 transition-all shadow-xl">Nuevo Programa</button>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
              {savedCourses.length === 0 && (
                <div className="col-span-full text-center py-32 bg-slate-900/40 rounded-[60px] border border-dashed border-white/5">
                  ...
                  <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.3em]">Tu biblioteca está vacía</p>
                  <p className="text-slate-600 text-xs mt-2">Comienza diseñando un nuevo programa académico.</p>
                </div>
              )}
              {savedCourses.map(c => (
                <div key={c.id} onClick={() => setCurrentCourse(c)} className="glass-card p-10 rounded-[40px] border border-white/5 hover:border-cyan-500/50 cursor-pointer transition-all group hover:scale-[1.02] hover:shadow-2xl hover:shadow-cyan-500/5">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between text-[10px] font-bold uppercase text-slate-500">
                      <span>Progreso de Diseño</span>
                      <span className="text-cyan-400">{Math.round((c.units.filter(u => u.lessons.length > 0).length / c.units.length) * 100)}%</span>
                    </div>
                    <div className="flex gap-1.5 h-1.5">
                      {c.units.map((u, i) => (
                        <div key={i} className={`flex-1 rounded-full transition-all duration-700 ${u.lessons.length > 0 ? 'bg-cyan-500' : 'bg-slate-800'}`}></div>
                      ))}
                    </div>
                  </div>
                </div>
      ...
      <footer className="mt-auto py-10 border-t border-white/5 text-center">
        <p className="text-[9px] font-black text-slate-700 uppercase tracking-[0.5em]">ProfesorIA © 2025 - Sistema de Gestión de Contenidos Educativos</p>
      </footer>
    </div>
  );
}
