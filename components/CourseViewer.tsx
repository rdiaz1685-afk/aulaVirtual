...
const CourseViewer: React.FC<CourseViewerProps> = ({ course, onExit, onUpdateCourse }) => {
  ...
  const handleBuildUnit = async (idx: number) => {
    ...
    setBuildStatus(`Diseñando: ${unitToBuild.title}...`);
    ...
    try {
      ...
      alert(`Error en el Nodo: ${e.message}`);
    } finally {
...
<head>
    ...
    <title>Alumno - ${course.title}</title>
    ...
    <script type="module">
        ...
        function StudentPlayer() {
            ...
            const downloadEvidence = (lesson, block, text) => {
                const evidence = {
                    studentName: prompt("Nombre del Alumno:") || "Sin Nombre",
                    studentId: prompt("ID Alumno:") || "0000",
                    lessonTitle: lesson.title,
                    activityTitle: block.title,
                    content: text,
                    aiScore: 0,
                    aiFeedback: "Pendiente de revisión por docente.",
                    timestamp: Date.now()
                };
                ...
                a.href = url;
                a.download = \`Entrega_\${evidence.studentName.replace(/\\s+/g, '_')}.json\`;
                a.click();
            };

            return React.createElement('div', { className: 'flex w-full h-screen overflow-hidden' }, [
                // Sidebar
                React.createElement('aside', { className: 'w-80 bg-slate-950 border-r border-white/5 flex flex-col h-full shrink-0 shadow-2xl' }, [
                    React.createElement('div', { className: 'p-8 border-b border-white/5 bg-slate-900/40' }, [
                        React.createElement('p', { className: 'text-[9px] font-black text-cyan-500 uppercase mb-2 tracking-widest' }, 'Portal del Alumno'),
                        ...
                        COURSE_DATA.units.map((unit, uIdx) => 
                            React.createElement('div', { key: uIdx, className: 'space-y-2' }, [
                                React.createElement('button', { 
                                    ...
                                    className: \`w-full text-left p-3 rounded-xl transition-all \${activeUnitIdx === uIdx ? 'bg-white/5 border border-white/10' : 'opacity-40 hover:opacity-100'}\`
                                ...
                                activeUnitIdx === uIdx && unit.lessons && unit.lessons.map((l, lIdx) => 
                                    React.createElement('button', {
                                        ...
                                        l.title,
                                        completedLessons.has(l.id) && React.createElement('span', { className: 'text-[8px] font-black' }, '✓')
                                    ])
                                )
                            ])
                        )
                ...
                // Content
                React.createElement('main', { className: 'flex-1 overflow-y-auto bg-slate-950 custom-scrollbar p-10 lg:p-20' }, [
                    currentLesson ? React.createElement('div', { className: 'max-w-4xl mx-auto space-y-12 pb-40' }, [
                        React.createElement('div', { className: 'mb-16' }, [
                            ...
                            React.createElement('h2', { className: 'text-5xl font-black mt-6 uppercase tracking-tighter' }, currentLesson.title)
                        ]),
                        currentLesson.blocks.map((block, bIdx) => {
                            ...
                            return React.createElement('div', { key: bIdx, className: \`rounded-[40px] border overflow-hidden \${isActivity ? 'border-cyan-500/30 bg-slate-900/50' : isTest ? 'border-amber-500/30 bg-slate-900/40' : 'border-white/5 bg-slate-900/20'}\` }, [
                                React.createElement('div', { className: 'px-10 py-5 border-b border-white/5 bg-slate-950/50 flex justify-between items-center' }, [
                                    React.createElement('h3', { className: 'text-[10px] font-black uppercase text-slate-500' }, block.title),
                                ...
                                React.createElement('div', { className: 'p-10' }, [
                                    React.createElement('p', { className: 'text-lg text-slate-300 leading-relaxed whitespace-pre-line mb-8' }, block.content),
                                    
                                    isActivity && React.createElement('div', { className: 'mt-8 space-y-4' }, [
                                        React.createElement('textarea', { 
                                            placeholder: 'Escribe tu respuesta aquí...',
                                            className: 'w-full h-40 bg-black/40 border border-white/5 rounded-2xl p-6 text-white outline-none focus:border-cyan-500 transition-all',
                                        ...
                                        React.createElement('button', {
                                            ...
                                            className: 'px-8 py-3 bg-cyan-500 text-slate-950 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-cyan-400 transition-all'
                                    ...
                                    isTest && block.testQuestions && React.createElement('div', { className: 'mt-10 space-y-12 border-t border-white/5 pt-10' }, 
                                        block.testQuestions.map((q, qIdx) => {
                                            const key = \`\${activeLessonIdx}-\${bIdx}-\${qIdx}\`;
                                            ...
                                            return React.createElement('div', { key: qIdx, className: 'space-y-6' }, [
                                                ...
                                                React.createElement('div', { className: 'grid md:grid-cols-2 gap-4' }, 
                                                    q.options.map((opt, oIdx) => {
                                                        ...
                                                        let btnClass = "w-full text-left p-5 rounded-2xl border-2 transition-all text-xs font-bold ";
                                                        ...
                                                        if (isShowingFeedback) {
                                                            ...
                                                            btnClass += "bg-white/5 border-white/5 text-slate-400 hover:border-amber-500/50";
                                                        }
                        ...
                        // Botón Final de Lección
                        React.createElement('div', { className: 'mt-32 flex flex-col items-center' }, [
                            React.createElement('button', {
                                ...
                                className: \`px-20 py-8 rounded-[40px] font-black transition-all active:scale-95 text-[11px] tracking-[0.4em] uppercase shadow-2xl \${
                                    completedLessons.has(currentLesson.id) ? 'bg-emerald-500 text-slate-950' : 'bg-white text-slate-950 hover:bg-cyan-400'
                                }\`
                            }, completedLessons.has(currentLesson.id) ? '✓ Lección Finalizada' : 'Marcar como Completada')
                        ])
                    ]) : React.createElement('div', { className: 'text-center py-40 opacity-20' }, 'Selecciona una lección para comenzar.')
                ])
            ]);
        }
    ...
    <div className="flex h-screen bg-[#020617] overflow-hidden">
      ...
      <aside className="w-80 bg-slate-950 border-r border-white/5 flex flex-col h-full z-30 shadow-2xl">
        <div className="p-8 border-b border-white/5 bg-slate-900/40">
          <button onClick={onExit} className="text-[10px] font-black text-cyan-500 uppercase tracking-widest mb-4 hover:text-white transition-colors">← Salir</button>
          ...
          {lessons.length > 0 && (
            ...
            >
              ...
              <div className="text-left">
                ...
                <p className="text-xs font-black text-white uppercase">Buzón y Portafolio</p>
              </div>
            </button>
          )}

          <div className="space-y-4 pt-4">
            <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest ml-4">Unidades Disponibles</p>
            {units.map((unit, uIdx) => (
                  ...
                  <div className="ml-4 space-y-1 border-l border-white/10 pl-4 py-1">
                    {unit.lessons.map((l, lIdx) => (
                      <button 
                        key={lIdx} 
                        onClick={() => setActiveLessonIdx(lIdx)} 
                        className={`w-full text-left p-3 rounded-xl text-[10px] font-black uppercase transition-all ${
        ...
        <div className="p-6 bg-slate-950 border-t border-white/10 space-y-2">
          ...
          <button onClick={handleExportHTML} className="w-full py-4 bg-white text-slate-950 rounded-2xl font-black uppercase text-[9px] tracking-widest hover:bg-slate-200 transition-all">EXPORTAR AULA (HTML)</button>
        </div>
      </aside>
        ...
        <div className="p-10 lg:p-20">
          {isBuildingUnit ? (
            <div className="max-w-xl mx-auto py-40 text-center">
               ...
               <h2 className="text-3xl font-black text-white uppercase mb-4 tracking-tighter">Generando Ingeniería</h2>
            ...
            <div className="max-w-xl mx-auto py-40 text-center bg-slate-900/20 rounded-[50px] border border-white/5">
               ...
               <p className="text-slate-500 uppercase text-[10px] font-black tracking-widest">Presiona el botón "CONSTRUIR UNIDAD" para empezar.</p>
            </div>
          )}
        </div>
      </main>
